...
final class CnpmRunner extends NodeTaskExecutor implements NpmRunner {
    static final String TASK_NAME = "cnpm";

    public CnpmRunner(NodeExecutorConfig config, ProxyConfig proxyConfig, String npmRegistryURL) {
        super(config, TASK_NAME, config.getCnpmPath().getAbsolutePath(), buildArguments(proxyConfig, npmRegistryURL));
    }
